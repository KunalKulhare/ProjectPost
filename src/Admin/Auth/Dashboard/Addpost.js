...
import { doPost } from "../../../redux/actionCreators/postsActionCreator";
...
const Addpost = () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (description.length < 100) {
      return alert("Description should be of atleast 100");
    }
    if (title.trim().split(" ").length < 2) {
      return alert("Title should be of atleast 2 words");
    }
    if (image.size > 5242880) {
      return alert("Image should be less than or equal to 5 MB");
    }
    const data = {
      title,
      category,
